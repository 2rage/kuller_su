...
class ParseFilter {
	...
	function BB_Parse($source, $use_html = TRUE) {
		...
		if( ! $this->safe_mode ) {
			...
			$source = preg_replace( "#\[flash=([^\]]+)\](.+?)\[/flash\]#ie", "\$this->build_flash('\\1', '\\2')", $source );
			$source = preg_replace( "#\[media=([^\]]+)\]#ie", "\$this->build_media('\\1')", $source );
	...
	function decodeBBCodes($txt, $use_html = TRUE, $wysiwig = false) {
		...
		$txt = preg_replace( "#\[code\](.+?)\[/code\]#ies", "\$this->decode_code('\\1', '{$use_html}', '{$wysiwig}')", $txt );
	...
	function decode_code($txt = "", $use_html, $wysiwig) { 

		if ( !$wysiwig AND $this->edit_mode )	{

			$txt = str_replace( "&amp;", "&", $txt );
		}

		if( !$wysiwig AND $use_html ) {
			$txt = str_replace( "&lt;br /&gt;", "\n", $txt );
		}

		if ( $wysiwig == 1 AND $this->edit_mode ) {
			$txt = str_replace( "\n", "&lt;br /&gt;", $txt );
			return "[code]".$txt."[/code]";
		}

		if ( $wysiwig == 2 AND $this->edit_mode ) {
			$txt = str_replace( "\n", "&lt;/p&gt;&lt;p&gt;", $txt );
