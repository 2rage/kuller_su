...
ipb.lang['ck_bbcode_error_title'] = "Ошибка BB-кода";
ipb.lang['ck_bbcode_desc']		  = "В вашем сообщении были найдены ошибки, пожалуйста исправьте их. Ошибки чаще всего возникают при несовпадении открывающих и закрывающих тэгов или отсутствии закрывающего тэга..<br />You may use your post anyway but it may not display as intended.<br />Tags with potential errors: #{tags}";
ipb.lang['ck__bbcode_use_anyway'] = "Все равно продолжить";
ipb.lang['ck__bbcode_close']      = "Закрыть и продолжить редактирование";
/*
		...
		text = text.replace( /(<blockquote class=['"]ipsBlockquote['"]\s*(data-author=['"](.+?)['"])?\s*(?:data-cid=['"]\d+?['"])?\s*(data-time=['"](\d+?)[\'"])?\s*>)/g, function ( whole, blockquote, _author, author, _date, date, offset )
		{
			...
			if ( date )
			{
				if ( date == parseInt( date ) && date.length == 10 )
				{
					...
					var _date = new Date().getDateAndTime( parseInt( date ) + parseInt( _tz ) );
			...
			for ( var i = 0; i < citeTags.count() ; i++ )
			{
				...
				if ( tag.hasClass('ipb') ) {
					tag.on( 'mouseup', function() {
						sel.selectElement( tag.getParent() );
						});
				}
			}
		}
		...
		CKEDITOR.on( 'instanceReady', function( ev )
		{
			if ( ev.editor.name == this.editorId )
			{
				...
				ev.editor.on( 'key', function( e ) {
					...
					if ( e.data.keyCode == 8 || e.data.keyCode == 46 || e.data.keyCode == 37 || e.data.keyCode == 38 )
					{
