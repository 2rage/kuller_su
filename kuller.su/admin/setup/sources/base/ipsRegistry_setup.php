 ...
 * Last Updated: $Date: 2013-01-07 11:27:42 -0500 (Mon, 07 Jan 2013) $
 * </pre>
 *
 * @author 		Matt Mecham
...
class ipsRegistry
{
	...
	public static function init()
	{
		...
		if ( IPS_IS_UPGRADER )
		{
			...
			{
				print "Файл " . DOC_IPS_ROOT_PATH . "conf_global.php не найден!";
				exit();
			}
			...
			{
				print "Файл " . DOC_IPS_ROOT_PATH . "conf_global.php не содержит нужных настроек.";
				exit();
			}
		...
		if ( IPS_IS_UPGRADER )
		{
			...
			{
				/* Force log in */
				self::getClass('output')->setTitle( "Обновление: Ошибка" );
		...
		else
		{
			...
			{
				self::getClass('output')->setTitle( "Установка: Ошибка" );
				self::getClass('output')->setNextAction( '' );
...
class ips_DBRegistry extends ips_base_Registry
{
	...
	static public function getPrefix( $key='' )
	{
		...
		{
			throw new Exception( "Ключ соединения с базой данных $key не найден" );
		}
	}
...
class ips_MemberRegistry extends ips_base_Registry
{
	...
	protected static function init()
	{
		if ( self::$initiated !== TRUE )
		{
			...
			{
				print "Невозможно определить ваш IPv4 адрес";
				exit();
			}
