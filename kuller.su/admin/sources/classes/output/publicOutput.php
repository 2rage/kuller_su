...
#print '#\d+?' . $_st . preg_quote( $seoTitle, '#' ) . '(' . $_end . "$|" . $_end . "\w+?" . $_end . "$|" . preg_quote( $_blk, '#' ) . ")#";exit;
		...
		{
			$match_pattern = '#' . $_st . preg_quote( $seoTitle, '#' ) . '(' . $_end . "$|" . $_end . '\w+?' . $_end . "$|/\?|" . preg_quote( $_blk, '#' ) . ")#";
		}
		else
		{
			$match_pattern =  '#\d+?' . $_st . preg_quote( $seoTitle, '#' ) . '(' . $_end . "$|" . $_end . '\w+?' . $_end . "$|/\?|" . preg_quote( $_blk, '#' ) . ")#";
		}
		if ( ! preg_match( $match_pattern,  $_toTest ) )
		{
			if ( $return === TRUE )
			{
				return FALSE;
