...
class classes_archive_restore
{
	...
	public function restore( $pids=array() )
	{
		...
		$posts = $topicClass->getPosts( array( 'postId'          => $pids,
											   ...
											   'skipForumCheck'  => true,
                                               'postType' => array('sdelete', 'visible', 'hidden', 'pdeleted')
		...
		*/
        ...
        {
          $maxes[ $t ] = intval( $this->engine->getMaxPidInTid( $t ) );
        }
		...
		/* Flag as archived */
		$this->engine->setAsRestoredByPids( array_keys ( $posts ) );
		...
		/* Log it */
		$this->log( array_keys ( $posts ), true );
	}
