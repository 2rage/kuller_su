	...
	protected function _finishUrlsForDisplay( $txt )
	{
		...
		for( $i = 0 ; $i < count( $urlMatches[0] ) ; $i++ )
		{
			...
			if ( $done !== true )
			{
				...
				{
					$text = htmlspecialchars( IPSText::mbsubstr(  $this->utf8_urldecode( $text ) , 0, 20 ) ) . '...' . htmlspecialchars( IPSText::mbsubstr($this->utf8_urldecode( $text ) , -15 ) );
				...
				$_title = '';
				$int_domains=array();
				$int_domains = explode( "\n", str_replace( "\r", "", $this->settings['nofollow_exclude'] ) );
				...
				$data = @parse_url( $url );
				//$data['host']=str_replace('www.', '',$data['host']);
				$int_domains[]=$_SERVER['HTTP_HOST'];
				$link_is_internal=FALSE;
				if ( count( $int_domains ) )
				{
					foreach($int_domains as $int_domain )
				{
						if( !trim($int_domain) )
					{
							continue;
						}
						$int_domain = preg_quote( $int_domain, '/' );
						$int_domain = str_replace( '\*', "(.*?)", $int_domain );
						if( preg_match( '/' . $int_domain . '/i', $data['host'] ) )
						{
				...
				if ( $this->settings['links_external'] )
				{
					...
					if ($this->settings['posts_links_redirect'])
					{				
	...
	function utf8_urldecode($str)
		{
			...
			$str = mb_convert_encoding($str, IPS_DOC_CHAR_SET);
			return html_entity_decode($str,null,IPS_DOC_CHAR_SET);
		}
	/**
	 * HTML Auto link URLs
	 * @param string $html
