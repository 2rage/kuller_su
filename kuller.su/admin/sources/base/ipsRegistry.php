 ...
 * Last Updated: $Date: 2013-01-17 09:16:27 -0500 (Thu, 17 Jan 2013) $
 * </pre>
 *
...
class ipsRegistry
{
	...
	static public function init()
	{
		...
		{	
			...
			ipsRegistry::$settings['noCacheKey'] = md5('$Rev: 11849 $');
			...
			if ( self::$handles['member']->getProperty('member_banned') == 1 or self::$handles['member']->getProperty( 'temp_ban' ) )
			{
				...
				if ( ipsRegistry::$request['module'] != 'ajax' or (ipsRegistry::$request['section'] != 'warnings' and ipsRegistry::$request['section'] != 'notifications' ))
				{
...
class seoTracker
{
	...
	protected static $engines  = array(
		...
		array( 'name' => 'Yandex', 'match' => '/http\:\/\/(www\.)?yandex/', 'parser' => 'parseYandexUrl'),
		array( 'name' => 'Rambler', 'match' => '/http\:\/\/(nova\.)?rambler/', 'parser' => 'parseRamblerUrl'),
		array( 'name' => 'Mail.ru', 'match' => '/http\:\/\/(go\.)?mail/', 'parser' => 'parseMailruUrl')
	);
	...
	protected function parseRamblerUrl($url)
	{
		$matches = array();
		...
		if(preg_match('/words\=([^\&]+)/', $url, $matches))
		{
			if(isset($matches[1]))
			{
				$keywords = $matches[1];
	...
	protected function parseMailruUrl($url)
	{
		$matches = array();
		if(preg_match('/q\=([^\&]+)/', $url, $matches))
		{
