...
EOF;
	...
	{	
		...
		if ( $data['set_by_skin_gen'] )
		{
			...
			{
				$canDelete = false;
				$desc .= "<a href='{$this->settings['base_url']}{$this->form_code}&amp;do=cancelVisualEditor&amp;set_id={$data['set_id']}' class='mini_button secondary right ipsCancelEditor'>Закрыть визуальный редактор</a>";
			}
			else
			{
				$desc .= "<a href='{$this->settings['base_url']}{$this->form_code}&amp;do=launchVisualEditor&amp;set_id={$data['set_id']}' target='_blank' class='mini_button right'>Запустить визуальный редактор</a>";
			}
		}
...
$IPBHTML .= <<<EOF
	...
	<script type="text/javascript">
		var cancelEditorDialogue = new Template("<div class='acp-box'><h3>Закрыть визуальный редактор</h3><div class='pad'><p>Данное действие закроет активную сессию визуального редактора, при этом будут потеряны все несохраненные изменения в стиле.<br /><br /><a href='#{url}' class='button'>Закрыть</a></p></div></div>" );
