...
class admin_core_languages_manage_languages extends ipsCommand
{
			...
			try
			{
				foreach( new DirectoryIterator( $_PATH ) as $f )
				{
					if ( preg_match( "#" . $_word . "_(.+?)_language_pack.xml#", $f->getFileName() ) )
					{
						...
						$this->imprtFromXML( 1, true, true, $next['key'], false, 1 );
					}
				}
			} catch ( Exception $e ) {}
	...
	public function handleWordEntryForm( $mode='add' )
	{
		...
		$this->request['word_app']		= mb_strtolower($this->request['word_app']);
		$this->request['word_pack_db']	= str_replace( '/', '_', mb_strtolower($this->request['word_pack_db']) );
	 ...
	 * @return	mixed
	 */
	public function imprtFromXML( $lang_id=0, $in_dev=0, $no_return=0, $app_override='', $skip_charset=false, $ru = '' )
	{
		//-----------------------------------------
		// Set version..
		//-----------------------------------------
		...
		foreach( $xml->fetchElements('lang') as $entry )
		{
			...
			foreach( $lang_ids as $_lang_id )
			{
				...
				$db_array = array(
									...
									'word_custom'			=> $in_dev && !$ru ? '' : $lang['word_custom'],
	...
	public function languageExportToXML( $lang_id, $disk=false )
	{
		...
		while( $r = $this->DB->fetch() )
		{
			...
			if( $disk )
			{
				...
				if( $r['word_custom'] )
				{
					$content['word_custom']=$r['word_custom'];
					$content['word_custom_version']	= ($r['word_custom_version'])?$r['word_custom_version']:( $r['word_default_version'] )?$r['word_default_version']:$LATESTVERSION['long'];
				}
