...
class admin_core_ajax_livesearch extends ipsAjaxCommand 
{
	...
	protected function _getMembers( $term, $results )
	{
		...
		while( $member = $this->DB->fetch() )
		{
			...
			if( $member['members_l_display_name'] AND mb_strpos( $term, $member['members_l_display_name'] ) !== false )
			{
				$r['_matched']	= 'members_display_name';
			}
			else if( $member['members_l_username'] AND mb_strpos( $term, $member['members_l_username'] ) !== false )
			{
				$r['_matched']	= 'name';
			}
			else
	...
	protected function _checkGroups( $term, $results )
	{
		...
		{
			if( strpos( $k, 'gf_' ) !== false AND mb_strpos( $v, $term ) !== false )
			{
		...
		if ( is_array( $groups ) AND count( $groups ) )
		{
			foreach( $groups as $id => $data )
			{
				...
				if ( preg_match( "#" . $_term . "#iu", $data['g_title'] ) )
				{
					$results['groups'][] = array(
	...
	protected function _checkForums( $term, $results )
	{
		...
		foreach( $forums->forum_by_id as $id => $data )
		{
			...
			if ( preg_match( "#" . $_term . "#iu", $data['name'] ) )
			{
				$data['extra'] = ( ( $data['posts'] == null ) ? 0 : $data['posts'] ) . " сообщений";
	...
	protected function _checkNexus( $term, $results )
	{
		...
		$textTerm		= $this->DB->addSlashes( mb_strtolower( $term ) );
		$intTerm		= intval( $term );
				...
				foreach( $cache as $entry )
				{
					if( count($entry['items']) )
					{
						foreach( $entry['items'] as $item )
						{
							...
							if( isset($item['keywords']) AND mb_stripos( $item['keywords'], $term ) !== false )
								{
									$item['name'] = $item['title'];
									$item['url'] = $this->settings['_base_url']."&amp;app={$app}&amp;module={$item['module']}&amp;{$item['url']}";
									$results['location'][] = $item;
								}
							else if( mb_stripos( $item['title'], $term ) !== false )
								{
									$item['name'] = $item['title'];
									$item['url'] = $this->settings['_base_url']."&amp;app={$app}&amp;module={$item['module']}&amp;{$item['url']}";
									$results['location'][] = $item;
