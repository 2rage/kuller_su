...
class admin_core_settings_settings extends ipsCommand
{
	...
	public function _findSettingGroup()
	{
		...
		while( $r = $this->DB->fetch() )
		{ 	
			if(  ( $r['conf_title_keyword'] == $this->request['key'] ) OR mb_strtolower( str_replace( " ", "", trim( $r['conf_title_title'] ) ) ) == urldecode( trim( $this->request['key'] ) ) )
			{
	...
	public function _viewSettings()
	{
		...
		if( $search_key )
		{
			$keywords	= mb_strtolower($search_key);
	...
	protected function _settingsTitlesCheck()
	{
		...
		while( $row = $this->DB->fetch( $outer ) )
		{
			...
			{
				$new_keyword = mb_strtolower( preg_replace( '#[^\d\w]#', "", $row['conf_title_title'] ) );
