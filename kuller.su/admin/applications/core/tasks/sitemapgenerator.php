...
class task_item
{
	...
	{	
		...
		{	
			...
			// Ping Yandex:
			@$http->getFileContents('http://webmaster.yandex.ru/wmconsole/sitemap_list.xml?host='.$sitemapUrl);
			if($http->http_status_code != 200)
			{
				$rtn = false;
				$this->log( sprintf( $this->lang->words['smt_ping_fail'], "Yandex" ) );
			}
			else
			{
				$this->log( sprintf( $this->lang->words['smt_ping_ok'], "Yandex") );
			}
		}
