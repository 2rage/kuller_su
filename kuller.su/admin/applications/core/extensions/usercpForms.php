 ...
 * Last Updated: $Date: 2013-01-11 22:07:47 -0500 (Fri, 11 Jan 2013) $
 * </pre>
 *
...
class usercpForms_core extends public_core_usercp_manualResolver implements interface_usercp
{
	...
	public function getLinks()
	{
		...
		if ( IPSLib::vkontakte_enabled() === TRUE )
		{
			$array[] = array( 'url'    => 'area=vkontakte',
							  'title'  => ipsRegistry::instance()->getClass('class_localization')->words['m_vkontakte'],
							  'active' => $this->request['tab'] == 'core' && $this->request['area'] == 'vkontakte' ? 1 : 0,
							  'area'   => 'vkontakte'
							 );
		}
	...
	public function runCustomEvent( $currentArea )
	{
		...
		switch( $currentArea )
		{
			...
			break;
			case 'vkontakteRemove':
				$html = $this->customEvent_vkontakteRemove();
			break;
			case 'twitterRemove':
	...
	/**
	 * Custom Event: Remove vk link
	 *
	 * @return	@e void  
	 */
	public function customEvent_vkontakteRemove()
	{
		//-----------------------------------------
		// Check secure hash...
		//-----------------------------------------
		...
		$vkontakte    = new $classToLoad( $this->registry );
			
		//-----------------------------------------
		// Okay...
		//-----------------------------------------
		
		if ( $this->memberData['vk_uid'] )
		{
			/* Remove the link */
			IPSMember::save( $this->memberData['member_id'], array( 'core' => array( 'vk_uid' => 0, 'vk_token' => '' ) ) );
		}
	...
	public function saveSignature()
	{
		...
		if( isset($sig_restrictions[5]) and $sig_restrictions[5] !== '' )
		{
			...
			$testSig	= wordwrap( $signature, $this->settings['signature_line_length'], '</p>' );
			
			// http://community.invisionpower.com/tracker/issue-35105-signature-restriction-minor-bug
			$testSig	= preg_replace( '#^\s*(</p>)+#i', '', $testSig );
			$testSig	= preg_replace( '#(</p>)+?\s*$#i', '', $testSig );
			
			if( substr_count( $testSig, "</p>" ) >= $sig_restrictions[5] )
			{
