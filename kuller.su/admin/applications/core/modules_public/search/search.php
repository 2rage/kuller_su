 ...
 * IP.Board v3.4.2
 * Global Search
 * Last Updated: $Date: 2013-01-18 09:43:36 -0500 (Fri, 18 Jan 2013) $
 * </pre>
 *
...
class public_core_search_search extends ipsCommand
{
	...
	/**
	 * Checks search flood time
	 */
	protected function _floodCheck()
	{
		/* Search Flood Check */
		if( $this->memberData['g_search_flood'] )
		{
			...
			if( $last_search )
			{
					...
					else if( ( time() - $last_search ) <= $this->memberData['g_search_flood'] ) 
					{
						$this->registry->output->showError( sprintf( $this->lang->words['xml_flood'], $this->memberData['g_search_flood'] - ( time() - $last_search ) ), 10149 );
					}
					else
					{
						/* Reset the cookie */
