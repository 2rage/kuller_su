...
class messengerFunctions
{
	...
	public function checkAndReturnInvitedUsers( $names, $member=array(), $current=0 )
	{
		...
		foreach( $_members as $member_id => $_member )
		{
			...
			$inviteArrayByName[ mb_strtolower($_member['members_l_display_name']) ] = $_member['member_id'];
		...
		foreach( $names as $name )
		{
			...
			if ( ! in_array( mb_strtolower( $name ), array_keys( $inviteArrayByName ) ) )
			{
				$doesNotExist[] = $name;
			}
		}
