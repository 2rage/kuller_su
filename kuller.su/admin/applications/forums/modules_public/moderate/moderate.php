...
class public_forums_moderate_moderate extends ipsCommand
{
	...
	protected function _multiSplitTopic()
	{
		...
		else
		{
			...
			foreach( $tids as $_tid => $pids )
			{
				...
				if( $this->settings['etfilter_shout'] && mb_strtoupper($this->request['title']) == $this->request['title'])
				{
					if( function_exists('mb_convert_case') )
					{
						if( in_array( mb_strtolower( $this->settings['gb_char_set'] ), array_map( 'mb_strtolower', mb_list_encodings() ) ) )
						{
	...
	protected function _deletePost()
	{
		...
		$this->registry->output->redirectScreen( $this->lang->words['post_deleted'], $this->settings['base_url'] . "showtopic=" . $this->topic['tid'] . "&amp;page=" . intval($redirectPage), $this->topic['title_seo'], 'showtopic' );
	}
