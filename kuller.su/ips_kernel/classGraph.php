...
class classGraph
{
    ...
    public $options			= array( 'titlecolor'		=> '#000000',			// The color of the title
									 'title'			=> 'График',				// The title text (no title of empty)
									 ...
									 'maxlegendwidth'	=> 20,					// Percentage indicating the maximum width for the legend
									 ...
									 'xaxisskip'		=> 0,					// Number of x-axis bars to skip before drawing next one (only implemented for line graphs so far)
								   );
	...
	*/
	public function addLabels( $data = array() )
	{
		...
		foreach( $data as $k => $v )
		{
			...
			{
				define( 'IPS_DOC_CHAR_SET', '' );
			}

			$_cleanData[ $k ]	= @html_entity_decode( $v, ENT_QUOTES, IPS_DOC_CHAR_SET );
		}
		
		$this->data['xaxis'] = $_cleanData;
	}
	 ...
	 * @param	string		Hex color code to use for this series
	 * @return	@e void
	 */
	...
	public function display()
	{
        ...
        // Check font
        //-----------------------------------------
        if ( $this->options['font'] and is_readable( $this->options['font'] ) and function_exists('imagettfbbox') )
        {
            $this->use_ttf = 1;
        }
		...
		{
			$this->_error( 'Минимальная ширина графика - 400, а минимальная высота - 200.' );
			return false;
		}
		...
		switch ( $this->options['charttype'] )
		{
			...
			case 'Radar':
				...
				break;
			default:
				$this->_error( 'Указан неизвестный тип графика \''.$this->options['charttype'].'\'.' );
				return false;
		}
	...
	protected function _drawLegend()
	{
		if ( $this->options['showlegend'] )
		{
	        ...
	        $maxlegendwidth = ( $this->grapharea['x1'] / 100 ) * $this->options['maxlegendwidth'];
	        ...
	        foreach ( $this->legend as $k => $name )
	        {
	            ...
	            {
	            	$txtsize = array();
	                $lines[ $k ] = explode( "\n", wordwrap( $name, ceil( ( $maxlegendwidth - 10 ) / imagefontwidth($this->fontsize) ) ) );
	                foreach ( $lines[ $k ] as $line )
	                {
	                	$_txtsize = imagettfbbox('10', 0, $this->options['font'], $line );
	                	if ( $_txtsize[2]-$_txtsize[0] > $txtsize[2]-$txtsize[0] )
	                	{
	                		$txtsize = $_txtsize;
	                	}
	                }
	            ...
	            else
	            {
	                ...
	                $txtsize = ( $txtsize > $maxlegendwidth ) ? $maxlegendwidth : $txtsize;
	        ...
	        foreach ( $this->legend as $k => $name )
	        {
				...
				foreach ( $lines[ $k ] as $line )
				{
		            if ( $this->use_ttf )
		            {
		                ...
		                imagettftext($this->image, "10", 0, $legendx+20+1, $txty+5+1, $shadowcolor, $this->options['font'], $line );
		                imagettftext($this->image, "10", 0, $legendx+20  , $txty+5  , $textcolor  , $this->options['font'], $line );
		            }
		            else
		            {
		                ImageString( $this->image, $this->fontsize, $legendx+20 , $ypos+7, $line, $this->black);
		            }
		...
		if ( isset( $this->x_axis['type'] ) )
		{
			...
			else
			{
				...
				for ( $i = 0; $i < $numlabels; $i++ )
				{
	                ...
	                if( $this->options['xaxisskip'] )
	                {
	                	if( $i % $this->options['xaxisskip'] != 1 )
	                	{
	                		continue;
	                	}
                	}
			...
			for ( $i=0; $i<$numbars; $i++ )
			{
				...
				if ( $this->options['showdatalabels'] )
				{
		            ...
		            if ( $this->use_ttf )
		            {
		                ...
		                $textx       = $x2 - ( round(($txtsize[2]-$txtsize[0])/2,0) * ( strlen($value2)/1.2 ) );
		                $texty       = $y2 - 3;
	 ...
	 * @param	boolean		Check labels exist
	 * @return	@e boolean
	 */
	protected function _checkSeries( $minseries=0, $labelsRequired=0 )
	{
		...
		{
			$this->_error( 'Должно быть не менее '.$minseries.' меток для добавления на \''.$this->options['charttype'].'\' график.' );
			return false;
		}
		if ( $labelsRequired && ( !isset( $this->data['xaxis'] ) or !is_array( $this->data['xaxis'] ) ) )
		{
			$this->_error( 'Необходимо добавить метки на график.' );
			return false;
		}
		...
		foreach( $this->data['yaxis'] as $series )
		{
			...
			{
				$this->_error( 'Не все ряды имеют одинаково колчиество чисел.' );
				return false;
			}
		}
		if ( $labelsRequired && count( $this->data['xaxis'] ) != count( $this->data['yaxis'][0]['data'] ) )
		{
			$this->_error( 'Число меток не совпадает с числом рядов.' );
			return false;
		}
		foreach( $this->data['yaxis'] as $series )
		{
			foreach( $series['data'] as $value )
			{
				if ( !is_numeric( $value ) )
				{
					$this->_error( 'Обнаружено не числовое значение. Данные в рядах должны быть числовыми.' );
					return false;
				}
			}
		}
	...
	protected function _checkColor( $color='' )
	{
		if ( strlen( $color ) != 7 )
		{
			$this->_error( 'Указан не верный код цвета. Код цвета должен быть указан в шестнадцатиричном формате. Например #FFFFFF для белого.' );
			return false;
		}
