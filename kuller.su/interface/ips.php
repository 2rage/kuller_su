 ...
 * $License: $
 * $Author: mark $
 * $LastChangedDate: 2012-11-09 10:11:12 -0500 (Fri, 09 Nov 2012) $
 * $Revision: 11584 $
	...
	if ( ipsRegistry::$settings['ipb_reg_number'] and ipsRegistry::$request['key'] == ipsRegistry::$settings['ipb_reg_number'] )
	{
		switch ( ipsRegistry::$request['do'] )
		{
			...
			case 'cdnoff':
				...
				{
					if ( preg_match( '#^http://p(\d+).ipscdn.com#', ipsRegistry::$settings['idm_screenshot_url'] ) )
					{
						$settings['idm_screenshot_url'] = preg_replace( '#^http://p(\d+).ipscdn.com#', ipsRegistry::$settings['board_url'], ipsRegistry::$settings['idm_screenshot_url'] );
					}
				}
				if ( IPSLib::appIsInstalled('gallery') )
				{
					if ( preg_match( '#^http://p(\d+).ipscdn.com#', ipsRegistry::$settings['gallery_images_url'] ) )
					{
						$settings['gallery_images_url'] = preg_replace( '#^http://p(\d+).ipscdn.com#', ipsRegistry::$settings['board_url'], ipsRegistry::$settings['gallery_images_url'] );
					}
				}
